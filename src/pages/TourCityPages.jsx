...
export default function TourCityPages() {
  const [tourCities, setTourCities] = useState([]);
  const [loading, setLoading] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingCity, setEditingCity] = useState(null);
  const [cityForm, setCityForm] = useState(initialTourCityForm);
  ...
  const fetchTourCities = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/admin/tour-cities');
      setTourCities(response.data);
    } catch (error) {
      setSnackbar({ open: true, message: 'Failed to fetch tour cities', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('tourTypes', JSON.stringify(cityForm.tourTypes));
      ...
      if (editingCity) {
        await api.put(`/api/admin/tour-cities/${editingCity._id}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setSnackbar({ open: true, message: 'Tour city updated successfully!', severity: 'success' });
      } else {
        await api.post('/api/admin/tour-cities', formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setSnackbar({ open: true, message: 'Tour city created successfully!', severity: 'success' });
      }
      
      setDialogOpen(false);
      setCityForm(initialTourCityForm);
      setEditingCity(null);
      fetchTourCities();
    } catch (error) {
      const message = error.response?.data?.message || 'Failed to save tour city';
  ...
  const handleEdit = (city) => {
    ...
    setCityForm({
      ...
      image: null,
      tourTypes: Array.isArray(city.tourTypes) ? city.tourTypes.map(t => typeof t === 'string' ? t : t.type || t) : [],
  ...
  const handleDelete = async (cityId) => {
    try {
      setLoading(true);
      await api.delete(`/api/admin/tour-cities/${cityId}`);
      setSnackbar({ open: true, message: 'Tour city deleted successfully!', severity: 'success' });
      fetchTourCities();
    } catch (error) {
      setSnackbar({ open: true, message: 'Failed to delete tour city', severity: 'error' });
    } finally {
      setLoading(false);
  ...
  const handleStatusToggle = async (cityId, currentStatus) => {
    try {
      await api.put(`/api/admin/tour-cities/${cityId}`, { isActive: !currentStatus });
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f7f9fb' }}>
      ...
      <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
              ...
              {tourCities.map((city) => (
                <TableRow key={city._id}>
                  ...
                  <TableCell>
                    ...
                    <Typography variant="body2" color="primary" sx={{ fontFamily: 'monospace', fontSize: '12px' }}>
                      /tour/{city.name.toLowerCase().replace(/\s+/g, '-')}
                    </Typography>
                  </TableCell>
                  <TableCell>
                    <FormControlLabel
        ...
        >
          <DialogTitle>
            {editingCity ? 'Edit Tour City' : 'Add New Tour City'}
          </DialogTitle>
          <DialogContent>
            <Box component="form" onSubmit={handleSubmit} sx={{ pt: 2 }}>
              ...
              <RichTextEditor
                ...
                label="Description *"
                placeholder="Enter a detailed description about the city and tour services..."
                height={250}
              ...
              <RichTextEditor
                ...
                placeholder="Enter the main content that will be displayed on the tour city page..."
                height={400}
                helperText="This content will appear on the tour city page. You can add headings, images, videos, links, tables, and formatted text."
              />

              <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth>
                    <InputLabel>Tour Types</InputLabel>
                    <Select
                      multiple
                      value={cityForm.tourTypes}
                      onChange={handleTourTypesChange}
                      input={<OutlinedInput label="Tour Types" />}
        ...
        >
          ...
          <DialogContent>
            <Typography>
              Are you sure you want to delete this tour city? This action cannot be undone.
            </Typography>
          </DialogContent>
          <DialogActions>
