...
const KathgodamTaxiPage = ({ cityData }) => {
  const [taxis, setTaxis] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [location, setLocation] = useState('Kathgodam');
  ...
  useEffect(() => {
    ...
    const fetchCities = async () => {
      try {
        ...
        setAllLocations(['Kathgodam']); // Fallback
      }
    };
    fetchCities();
  }, []);
  ...
  useEffect(() => {
    const fetchTaxis = async () => {
      try {
        setLoading(true);
        const res = await api.get('/api/taxis', { params: { isAvailable: true, location: 'Kathgodam' } });
        setTaxis(res.data);
      } catch (err) {
    ...
    <>
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Taxis in Kathgodam</h1>
              <p className="text-gray-600">Find the perfect taxi for your journey in Kathgodam</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center text-gray-500 py-8">
                <p>No taxis available in Kathgodam at the moment.</p>
              </div>
            ) : (
...
export default KathgodamTaxiPage;
