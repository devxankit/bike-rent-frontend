...
const AlmoraBikesPage = ({ cityData }) => {
  const [bikes, setBikes] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [location, setLocation] = useState('Almora');
  ...
  useEffect(() => {
    const fetchBikes = async () => {
      try {
        setLoading(true);
        const res = await api.get('/api/bikes', { params: { isBooked: false, location: 'Almora' } });
        setBikes(res.data);
      } catch (err) {
  ...
  useEffect(() => {
    if (location && location.toLowerCase() !== 'almora') {
    ...
    <>
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Bikes in Almora</h1>
              <p className="text-gray-600">Find the perfect bike for your journey in Almora</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center text-gray-500 py-8">
                <p>No bikes available in Almora at the moment.</p>
              </div>
            ) : (
...
export default AlmoraBikesPage;
