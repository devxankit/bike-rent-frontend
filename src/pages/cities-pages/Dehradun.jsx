...
const DehradunBikesPage = () => {
  ...
  useEffect(() => {
    const fetchBikes = async () => {
      try {
        setLoading(true);
        const res = await api.get('/api/bikes', { params: { isBooked: false, location: 'Dehradun' } });
        setBikes(res.data);
      } catch (err) {
  ...
  useEffect(() => {
    if (location && location.toLowerCase() !== 'dehradun') {
      const city = location.trim().toLowerCase();
      if (["indore", "bhopal", "mumbai", "goa", "haldwani", "kathgodam", "pithoragarh"].includes(city)) {
    ...
    <>
      ...
      <div className="flex min-h-screen h-screen bg-gray-50">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Bikes in Dehradun</h1>
              <p className="text-gray-600">Find the perfect bike for your journey in Dehradun</p>
            </div>
            {loading ? (
              ...
              <div className="text-center text-gray-500 py-8">
                <p>No bikes available in Dehradun at the moment.</p>
              </div>
            ) : (
...
export default DehradunBikesPage; 