...
const TourNavbar = () => {
  ...
  if (location.pathname.startsWith('/tours/about')) aboutDropdownLabel = 'About';
  else if (location.pathname.startsWith('/tours/contact')) aboutDropdownLabel = 'Contact Us';
  else if (location.pathname.startsWith('/tours/privacy-policy')) aboutDropdownLabel = 'Privacy Policy';
  else if (location.pathname.startsWith('/tours/terms-and-conditions')) aboutDropdownLabel = 'Terms&Conditions';
  ...
  const drawerLinks = (
    <nav className="flex flex-col gap-4 mt-8 z-[10000]">
      ...
      <Link to="/tours/about" className={`text-lg font-semibold ${getActiveLinkStyles(isActiveLink('/tours/about'))}`} onClick={() => setDrawerOpen(false)}>About</Link>
      ...
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-14">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <div className="relative" ref={aboutDropdownRef}>
            <button
              className={`text-sm font-semibold ${getActiveLinkStyles(isActiveLink('/tours/about') || isActiveLink('/tours/contact') || isActiveLink('/tours/privacy-policy') || isActiveLink('/tours/terms-and-conditions'))} flex items-center gap-1 focus:outline-none px-3 py-1 rounded transition-all ${aboutDropdownOpen ? 'bg-yellow-50' : ''}`}
              type="button"
            ...
            {aboutDropdownOpen && (
              <div className="absolute left-0 mt-2 w-40 bg-white border border-gray-200 rounded shadow-lg z-20">
                <Link to="/tours/about" className={`block px-4 py-2 text-sm ${getActiveLinkStyles(isActiveLink('/tours/about'))} hover:bg-yellow-50`} onClick={() => setAboutDropdownOpen(false)}>About</Link>
                <Link to="/tours/contact" className={`block px-4 py-2 text-sm ${getActiveLinkStyles(isActiveLink('/tours/contact'))} hover:bg-yellow-50`} onClick={() => setAboutDropdownOpen(false)}>Contact Us</Link>
                <Link to="/tours/privacy-policy" className={`block px-4 py-2 text-sm ${getActiveLinkStyles(isActiveLink('/tours/privacy-policy'))} hover:bg-yellow-50`} onClick={() => setAboutDropdownOpen(false)}>Privacy Policy</Link>
                <Link to="/tours/terms-and-conditions" className={`block px-4 py-2 text-sm ${getActiveLinkStyles(isActiveLink('/tours/terms-and-conditions'))} hover:bg-yellow-50`} onClick={() => setAboutDropdownOpen(false)}>Terms&Conditions</Link>
              </div>
            )}
          </div>
          {isAdmin && (
